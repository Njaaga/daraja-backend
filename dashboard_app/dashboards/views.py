...
class ApiDataSourceViewSet(viewsets.ModelViewSet):
    ...
    def entity_fields(self, request, pk=None, entity=None):
        ...
        Endpoint: /api/api-sources/{id}/entities/{entity}/fields/
        """
        ...
        if getattr(api_source, "oauth_token_expires_at", None) and api_source.oauth_token_expires_at <= timezone.now():
            try:
                refresh_quickbooks_token(api_source)
            except requests.RequestException as e:
                return Response(
                    {"error": "Failed to refresh QuickBooks token", "details": str(e)},
        ...
        qb_entity = ENTITY_MAP.get(entity.lower())
        if not qb_entity:
            return Response(
        ...
        query = f"SELECT * FROM {qb_entity} STARTPOSITION 1 MAXRESULTS 1"
        ...
        # Execute query
        try:
            resp = requests.post(qb_query_url, data=query, headers=headers, timeout=60)
            ...
            return Response(
                {"error": "Invalid JSON response from QuickBooks", "details": str(e)},
                status=status.HTTP_502_BAD_GATEWAY
            )
    
        # Check for QB logical errors
        if "Fault" in payload:
            return Response(
        ...
        fields = sorted(set(extract_fields(records[0])))
...
class DatasetViewSet(viewsets.ModelViewSet):
    ...
    def _run_dataset(self, dataset):
        ...
        Supports QuickBooks (via SELECT *) and generic REST APIs.
        """
        ...
        if source.provider.lower() == "quickbooks":
            ...
            # --------------------------------------------------
            # BUILD SAFE QUERY (ALWAYS SELECT *)
            # --------------------------------------------------
            ...
            if not query_response:
                return Response(
                    {"entity": entity, "fields": fields, "count": 0, "data": []}
                )
            ...
            # --------------------------------------------------
            # FIELD EXTRACTION (SUPPORTS NESTED FIELDS)
            # --------------------------------------------------
            ...
            data = [extract_fields(r) for r in rows]
...
class ChartViewSet(viewsets.ModelViewSet):
    ...
    def _run_dataset(self, dataset):
        """
        Executes a dataset against its data source (QuickBooks or generic REST API).
        Supports:
        - QuickBooks POST /query
        - Generic REST GET
        Handles:
        - Field filtering
        ...
        if source.provider.lower() == "quickbooks":
            ...
            # ---------------- FETCH RAW ROWS ----------------
            # Select only fields allowed by QuickBooks
            allowed_fields = self._get_qb_allowed_fields(entity)  # implement to fetch or cache QB entity fields
            safe_fields = [f for f in fields if f in allowed_fields]
    
            query_fields = ", ".join(safe_fields) if safe_fields else "*"
            query = f"SELECT {query_fields} FROM {entity} STARTPOSITION 1 MAXRESULTS 100"
            url = f"{source.base_url}/query"
            ...
            data = [{f: pick(r, f) for f in safe_fields} for r in rows]
            ...
            if aggregation and agg_field and not group_by:
                values = [float(row.get(agg_field) or 0) for row in data if row.get(agg_field) is not None]
                result = {
                    "sum": sum(values),
                    "count": len(values),
                ...
                return Response({"type": "kpi", "field": agg_field, "aggregation": aggregation, "value": result})
            ...
            # Table
            return Response({"type": "table", "count": len(data), "data": data})
